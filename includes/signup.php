    ...
    if (isset($_POST['next'])) {
        ...
        if (!empty($username)) {
            if (strlen($username) > 20) {
                ...
                $error =
                    'Username must be longer than 6 alphanumeric characters without any spaces';
            } elseif ($getFromU->checkUsername($username) === true) {
                $error = 'Username is already taken!';
            } else {
                $getFromU->update('users', $user_id, ['username' => $username]);
                header('Location: ' . ROOT_URL . 'signup.php?step=2');
            }
        } else {
    ...
    <?php if ($_GET['step'] == '2') { ?>
  	<div class='lets-wrapper'>
  		<div class='step-letsgo'>
  			...
  			<span>
  				<a href='<?= ROOT_URL ?>home.php' class='backButton' style="color:var(--twitter-color);">Let's go!</a>
  			</span>
  		</div>
  	</div>
  <?php } ?>
