...
class User
{
    ...
    public function search($search)
    {
        $stmt = $this->pdo->prepare(
            'SELECT `user_id`,`username`,`screenName`,`profileImage`,`profileCover` FROM `users` WHERE `username` LIKE ? OR `screenName` LIKE ?'
        );
    ...
    public function register($email, $password, $screenName)
    {
        $passwordHash = md5($password);
        $stmt = $this->pdo->prepare(
            "INSERT INTO `users` (`email`, `password`, `screenName`, `profileImage`, `profileCover`) VALUES (:email, :password, :screenName, 'assets/images/defaultprofileimage.png', 'assets/images/defaultCoverImage.png')"
        );
        $stmt->bindParam(':email', $email, PDO::PARAM_STR);
        $stmt->bindParam(':password', $passwordHash, PDO::PARAM_STR);
        ...
        if (in_array($ext, $allowed_extensions)) {
            if ($errors === 0) {
                if ($fileSize <= 2097152) {
                    ...
                    move_uploaded_file(
                        $fileTmp,
                        $_SERVER['DOCUMENT_ROOT'] . '/twitter/' . $root
                    );
                    return $root;
                } else {
