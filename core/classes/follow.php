...
class Follow extends User
{
    ...
    public function checkFollow($followerID, $user_id)
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM `follow` WHERE `sender` = :user_id  AND `receiver` = :followerID'
        );
    ...
    public function followBtn($profileID, $user_id, $followID)
    {
        ...
        if ($this->loggedIn() === true) {
            if ($profileID != $user_id) {
                if (
                    isset($data['receiver']) &&
                    $data['receiver'] === $profileID
                ) {
                    //Following btn
                    return "<button class='f-btn following-btn follow-btn' data-follow='" .
                        $profileID .
                        "' data-profile='" .
                        $followID .
                        "' style='outline:none;'>Following</button>";
                } else {
                    //Follow button
                    return "<button class='f-btn follow-btn' data-follow='" .
                        $profileID .
                        "' data-profile='" .
                        $followID .
                        "' style='outline:none;'><i class='fa fa-user-plus'></i>Follow</button>";
                }
            } else {
                //edit button
                return "<button class='new-btn' onclick=location.href='" .
                    ROOT_URL .
                    "profileEdit.php' style='outline:none;'>Edit Profile</button>";
            }
        } else {
            return "<button style='outline:none;' class='f-btn' onclick=location.href='" .
                ROOT_URL .
                "index.php'><i class='fa fa-user-plus'></i>Follow</button>";
        }
    }
    ...
    public function follow($followID, $user_id, $profileID)
    {
        ...
        $stmt = $this->pdo->prepare(
            'SELECT `user_id`, `following`, `followers` FROM `users` LEFT JOIN `follow` ON `sender` = :user_id AND CASE WHEN `receiver` = :user_id THEN `sender` = `user_id` END WHERE `user_id` = :profileID'
        );
        $stmt->execute(['user_id' => $user_id, 'profileID' => $profileID]);
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE `users` SET `following` = `following` + 1 WHERE `user_id` = :user_id; UPDATE `users` SET `followers` = `followers` + 1 WHERE `user_id` = :followID'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM `users` LEFT JOIN `follow` ON `receiver` = `user_id` AND CASE WHEN `sender` = :profileID THEN `receiver` = `user_id` END WHERE `sender` IS NOT NULL '
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM `users` LEFT JOIN `follow` ON `sender` = `user_id` AND CASE WHEN `receiver` = :profileID THEN `sender` = `user_id` END WHERE `user_id` and `receiver` IS NOT NULL'
        );
        $stmt->bindParam(':profileID', $profileID, PDO::PARAM_INT);
        $stmt->execute();
        $followings = $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM `users` WHERE `user_id` != :user_id AND `user_id` NOT IN (SELECT `receiver` FROM `follow` WHERE `sender` = :user_id) ORDER BY rand() LIMIT 3'
        );
